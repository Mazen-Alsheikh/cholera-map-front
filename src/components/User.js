...
function User() {
    ...
    useEffect(()=> {
        ...
        fetch(`cholera-map-back.railway.internal/api/user/${id}`, {method:"GET"})
        .then((res) => res.json())
        .then((data) => {
            setUser(data.user)
            setLoading(false)
    ...
    const handleDelete = () => {
        if (window.confirm("هل أنت متأكد أنك تريد حذف هذا المستخدم؟")) {
            fetch(`cholera-map-back.railway.internal/api/deleteUser/${id}`, { method: "POST" })
            .then(res => res.json())
            .then(data => {
                if (data.success) {
    ...
    const handleSave = () => {
        ...
        fetch(`cholera-map-back.railway.internal/api/updateuser/${id}`, {
            method: "POST",
        ...
        {!isEditing ? (
            <>
                ...
                <ListGroup className="list-group-flush">
                    <ListGroup.Item className="list-group-flush bg-dark text-light">الإيميل: {user.email}</ListGroup.Item>
                    <ListGroup.Item className="list-group-flush bg-dark text-light">الرتبة: {user.group_id === 1 ? "مدير" : "محرر"}</ListGroup.Item>
                </ListGroup>
