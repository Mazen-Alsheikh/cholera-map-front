...
function Profile() {
    ...
    useEffect(()=> {
        ...
        fetch(`cholera-map-back.railway.internal/api/profile/${id}`, {method:"GET"})
        .then((res) => res.json())
        .then((data) => {
            setUser(data.user)
            setLoading(false)
        ...
        {!isEditing ? (
            <>
                ...
                <ListGroup className="list-group-flush bg-dark text-light">
                    ...
                    <ListGroup.Item className="bg-dark text-light border-light">الرتبة: {user.role}</ListGroup.Item>
                </ListGroup>
                <Card.Body className='text-center'>
                    <Button className='ms-2' onClick={handleEditToggle} variant='info'>تعديل</Button>
                    {me?.role === "مدير" && <Button variant='danger' onClick={handleDelete}>حذف</Button>}
                </Card.Body>
            </>
        ) : (
